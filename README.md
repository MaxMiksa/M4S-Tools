...
一款适用于 Windows 平台的桌面 GUI 应用程序，用于处理 M4S 文件（通常来自流媒体下载）。用户可以通过图形界面选择文件，一键完成 M4S 视频片段合并、音频片段合并以及最终的音视频混流工作。
...
- **文件大小**：约 10-15MB（包含所有依赖）
- **运行要求**：
  - Windows 10/11
  - 首次运行需要网络连接（用于自动下载 FFmpeg）
  - 无需安装 Python 或其他依赖
...
- 通过 U盘、网盘等方式分享给其他用户
- 首次运行时，程序会自动检测并安装 FFmpeg（需要网络连接）

**注意**：如果目标电脑没有网络连接，用户需要手动安装 FFmpeg 并添加到 PATH。
